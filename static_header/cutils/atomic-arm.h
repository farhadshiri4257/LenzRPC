...
int32_t android_atomic_add(int32_t increment, volatile int32_t *ptr)
{
    ...
    android_memory_barrier();
    do {
        __asm__ __volatile__ ("ldrex %0, [%4]\n"   //load last value from (ptr-%4) into (prev-%0) memory address
                              "add %1, %0, %5\n"   //add (prev-%0) value with (incerment-%5) value resault to (tmp-%1) memory adddress
                              "strex %2, %1, [%4]" //try and claim the lock, If the exclusive monitor permit (0) write to(status-%2) otherwise write (1).
                              : "=&r" (prev), "=&r" (tmp),  //(prev-%0),(tmp-%1)
                                "=&r" (status), "+m" (*ptr) //(status-%2),(ptr-%4)
                              : "r" (ptr), "Ir" (increment) //(incerment-%5)
                              : "cc");
    } while (__builtin_expect(status != 0, 0));//infinit loop until status is 0, status is exclusive monitor
    return prev;
}
